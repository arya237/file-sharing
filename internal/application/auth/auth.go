...
func (u *UseCase) Register(ctx context.Context, input *RegisterInput) (*RegisterOutput, error) {
	...
	if username == "" {
		return nil, apperr.InvalidInput("auth", "username is required", nil)
	}

	if input.Password == "" {
		return nil, apperr.InvalidInput("auth", "password is required", nil)
	}

	passwordHash, err := u.passwordHasher.Hash(input.Password)
	if err != nil {
		return nil, apperr.Dependency("auth", "failed to hash password", err)
	}
...
func (u *UseCase) Login(ctx context.Context, input *LoginInput) (*LoginOutput, error) {
	...
	if username == "" || input.Password == "" {
		return nil, apperr.InvalidInput("auth", "username or password is required", nil)
	}

	existingUser, err := u.users.FindByUsername(ctx, username)
	if err != nil {
		if errors.Is(err, user.ErrNotFound) {
			return nil, apperr.Unauthorized("auth", "invalid username or password", nil)
		}
		return nil, apperr.Dependency("auth", "failed to find user", err)
	}
